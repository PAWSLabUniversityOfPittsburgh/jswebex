...
// All content packages in the given content type
    ...
    if (req.params.contentType in handlers.contentTypes) {
        ...
        for (var i = 0; i < packages.length; i++) {
            var currentContent = {};
            currentContent.name = packages[i].meta.name;
            currentContent.url = config.serverAddress + 'api/v1/content/' + req.params.contentType + '/' + packages[i].meta.name;
...
api.get('/content/:contentType([a-z]+)/:contentPackage([a-zA-Z0-9_-]+)', function(req, res) {
    ...
    if (req.params.contentPackage in handlers.contentPackages) {
        ...
        var allContent = handlers.contentPackages[req.params.contentPackage].meta.contents;
        var content = [];        
        ...
        Object.keys(allContent).forEach(function(key) {
            ...
            currentContent.html_url = config.serverAddress + 'html/' + req.params.contentType + '/' + req.params.contentPackage + '/' + currentContent.name;
...
api.get('/content/:contentType([a-z]+)/:contentPackage([a-zA-Z0-9_-]+)/:name([a-zA-Z0-9_-]+)', function(req, res) {
    ...
    if (req.params.contentPackage in handlers.contentPackages && req.params.name in handlers.contentPackages[req.params.contentPackage].meta.contents) {
    
        var exercise = handlers.contentPackages[req.params.contentPackage].meta.contents[req.params.name];
        ...
        exerciseInfo.html_url = config.serverAddress + 'html/' + req.params.contentType + '/' + req.params.contentPackage + '/' + req.params.name;
